...
def _find_dataset_root(root):
    """Find a folder under root that contains test_split.csv + images/."""
    for p in sorted(root.rglob('test_split.csv')):
        candidate = p.parent
...
def get_dataset():
    """Returns (test_df, image_dir), extracting dataset.zip if needed."""
    if EXTRACT_DIR.exists():
        found = _find_dataset_root(EXTRACT_DIR)
        if found:
            print(f"  Dataset already extracted → {found}")
            return pd.read_csv(found / 'test_split.csv'), found / 'images'
    ...
    if not found:
        raise FileNotFoundError(
            "Extraction succeeded but could not find test_split.csv + images/ "
            f"inside {EXTRACT_DIR}"
        )
...
def main():
    ...
    print("\n[Dataset]")
    test_df, image_dir = get_dataset()
    val_tfm      = transforms.Compose([
        transforms.ToTensor(), transforms.Normalize(_MEAN, _STD)])
    test_dataset = AptosDataset(test_df, image_dir, transform=val_tfm)
