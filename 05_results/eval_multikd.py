...
SCENARIOS = [
    {
        ...
        'ckpt_prefix': 'kd',                   # kd_seed{seed}_resume.pth
    },
    {
        ...
        'ckpt_prefix': 'coral_kd',             # coral_kd_seed{seed}_resume.pth
    },
    {
        ...
        'ckpt_prefix': 'hybrid_camkd',         # hybrid_camkd_seed{seed}_resume.pth
    },
    {
        ...
        'ckpt_prefix': 'hybridresnet50_camkd', # hybridresnet50_camkd_seed{seed}_resume.pth
    },
]
...
def load_student(ckpt_path):
    ...
    for builder, label in [(get_mobilenet_ce, 'CE'), (get_mobilenet_coral, 'CORAL')]:
        ...
        try:
            ...
            continue
    raise RuntimeError(
        f"Neither CE nor CORAL head matched.\n"
...
def predict_batch(model, imgs, head):
    ...
    return (torch.sigmoid(model.coral_head(x)) > 0.5).sum(dim=1).long()
...
def write_csv(csv_path, scenario_short, seed_results):
    ...
    cls_keys = [(cls, f'p_{cls.lower().replace(" ","_")}',
    ...
    with open(csv_path, 'a', newline='', encoding='utf-8') as f:
        ...
        for r in seed_results:
            row = {
                'scenario': scenario_short, 'seed': r['seed'], 'head': r['head'],
                'qwk':      round(r['qwk'],       4),
...
def main():
    ...
    print("\n" + "=" * W)
    print("  04_kd_multi — Student Evaluation")
    print("=" * W)
