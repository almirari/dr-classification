...
SCENARIOS = [
    {
        ...
        'ckpt_prefix':  None,        # seed{seed}_resume.pth
    },
    {
        ...
        'ckpt_prefix':  'student',   # student_seed{seed}_resume.pth
    },
]
...
def main():
    ...
    print("\n" + "=" * W)
    print("  03_kd_single — Student Evaluation")
    print("=" * W)
    ...
    for scenario in SCENARIOS:
        ...
        for seed in SEEDS:
            ...
            fname  = f"{prefix}_seed{seed}_resume.pth" if prefix else f"seed{seed}_resume.pth"
    ...
    if summary_rows:
        lines = ["=" * W,
                 "  SUMMARY — 03_kd_single  (mean ± std across seeds)",
                 "=" * W,
                 f"  {'Scenario':<30} {'QWK':>16} {'Acc (%)':>14} {'Macro F1':>14}",
                 "  " + "─" * (W-2)]
        for row in summary_rows:
            mq,sq = row['qwk']; ma,sa = row['acc']; mf,sf = row['macro_f1']
            lines.append(f"  {row['scenario']:<30} {mq:.4f}±{sq:.4f}   "
